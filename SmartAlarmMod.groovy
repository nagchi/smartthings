...
def pageSetup() {
    ...
    return dynamicPage(pageProperties) {
        ...
        section("Setup Menu") {
			...
			href "pageSelectIndicators", title:"Add/Remove Indicators", description:"Tap to open"
        }
...
def armAway() {
    LOG("armAway() atomicState.armed = ${atomicState.armed} , atomicState.stay = ${atomicState.stay}")
...
// Show "Add/Remove Indicators" page
...
private def initIndicators() {
    LOG("initIndicators()")
	
    if (settings.z_indicator) {
		subscribe(settings.z_indicator, "switch", "onIndicator")
    }
	indicatorUpdate()
}
